...
namespace WirelessNodeSimulation
{
	...
	public class WirelessSensorNetwork	{
      ...
      //  TracerForm obj = new TracerForm();
        ...
        public bool Process(bool bSleep)
        {
            ...
            vectors.Update();
            bool flg = true;
            while (vectors.aVectors.Count < 3&&flg)
            {
                int k = 1, kk = 1;
                foreach (WirelessSensor sensor in aSensors)
                {
                        ...
                        foreach (Vector vector in vectors.aVectors)
                        {
                            if (Math.Sqrt(Math.Pow(vector.x - sensor.x, 2) + Math.Pow(vector.y - sensor.y, 2)) < sensor.iSensorRadius)
                            {
                                ...
                                //   sensor.iResidualEnergy = WirelessSensorNetwork.iMaxEnergy;
                                    ...
                                    //obj.addpacketdetails(sensor.x.ToString(), sensor.y.ToString(), sensor.iSensorDelay.ToString());
                                }
                            }
                        }
                    }
		...
		public void BeginTransmission()
        {
			...
			if ((sSender.aPackets.Count > 0) && (iTransmitting == 0)) {
				if (sSender.iResidualEnergy <= iTransmitCost)  // transmission failed - sender has run out of energy
                   ...
                   // obj.addpacketdetails(packet.x.ToString(), packet.y.ToString(), "");
	...
	public class VectorList {
        ...
        public List<gridDisplay> GridPacketStatus = new System.Collections.Generic.List<gridDisplay>();
        public void routing()
        {
          
            string routex = Program.x, routey = Program.y;

            string[] xarray = routex.Split(',');
            string[] yarray = routey.Split(',');
            
            for (int i = 0; i < 5; i++)
            {
             int j = r.Next(xarray.Length - 1);
             int n = r.Next(xarray.Length-1);
             while (j == n)
            {
              n = r.Next(xarray.Length - 1);
            }
                aVectors.Add(new Vector(Convert.ToInt32(xarray[j]), Convert.ToInt32(yarray[j]),Convert.ToInt32(xarray[n]), Convert.ToInt32(yarray[n])));
                updateList(Convert.ToInt32(xarray[n]), Convert.ToInt32(yarray[n]), false);
             
                int m = r.Next(xarray.Length - 1);
                while (j == m)
                {
                    m = r.Next(xarray.Length - 1);
                }
                aVectors.Add(new Vector(Convert.ToInt32(xarray[j]), Convert.ToInt32(yarray[j]), Convert.ToInt32(xarray[m]), Convert.ToInt32(yarray[m])));
                updateList(Convert.ToInt32(xarray[m]), Convert.ToInt32(yarray[m]), false);
                

                updateList(Convert.ToInt32(xarray[j]), Convert.ToInt32(yarray[j]), true);
                
                
            }
        }

        public void updateList(int X,int Y,bool IsSend)
        {
            gridDisplay result = GridPacketStatus.Find(x => x.X == X && x.Y == Y);
            if (result != null)
            {
                if (IsSend)
                {
                ...
                {
                    result.ReceivedPacket += 1;
                    }
            }
            else
            {
                if(IsSend)
                GridPacketStatus.Add(new gridDisplay() { X = X, Y = Y, SendPacket = 2, ReceivedPacket = 0,BouncedPacket=0 });
                else
                    GridPacketStatus.Add(new gridDisplay() { X = X, Y = Y, SendPacket = 0, ReceivedPacket = 1,BouncedPacket =0 });
    ...
    public class gridDisplay
    {
        ...
        public int BouncedPacket { get; set; }
